...
class DiskDBClient:
    ...
    def _parse_array_response(self, command: str, expected_lines: int) -> List[str]:
        """
        Send command and parse multi-line array response.
        
        Args:
            command: Command to send
            expected_lines: Number of lines to read
        ...
        for _ in range(expected_lines):
            ...
            while True:
                ...
                if b"\n" in chunk:
                    break
            lines.append(response.decode().strip())
    ...
    def lrange(self, key: str, start: int, stop: int) -> List[str]:
        ...
        response = self._send_command(f"LRANGE {key} {start} {stop}")
        if response == "":
            return []
        # For multi-line responses, we need to handle them differently
        # First, check if it's an empty array
        command = f"LRANGE {key} {start} {stop}"
        
        # We need to know how many elements to expect
        # Let's use LLEN to get the list length first
        length_response = self._send_command(f"LLEN {key}")
        if length_response == "0":
            return []
        
        # Calculate expected number of elements
        list_len = int(length_response)
        if start < 0:
        ...
        expected_elements = max(0, stop - start + 1)
        if expected_elements == 0:
            return []
        
        return self._parse_array_response(command, expected_elements)
    ...
    def smembers(self, key: str) -> List[str]:
        ...
        # First get the set size
        card_response = self._send_command(f"SCARD {key}")
        card = int(card_response)
        if card == 0:
            return []
        
        # For SMEMBERS, we expect one member per line
        return self._parse_array_response(f"SMEMBERS {key}", card)
    ...
    def hgetall(self, key: str) -> Dict[str, str]:
        ...
        type_response = self._send_command(f"TYPE {key}")
        if type_response == "none":
            return {}
        
        # For HGETALL, we need to know how many field-value pairs to expect
        # We can't easily determine this, so we'll read until we get an empty response
        # For now, let's implement a simple version
        response = self._send_command(f"HGETALL {key}")
        
        # If empty hash
        if response == "":
            return {}
        
        # For simplicity, let's assume the response format and parse accordingly
        # In a real implementation, we'd need to handle the protocol properly
        result = {}
        # This is a simplified implementation
        # You might need to adjust based on actual server response format
        return result
    ...
    def zrange(self, key: str, start: int, stop: int, withscores: bool = False) -> Union[List[str], List[Tuple[str, float]]]:
        ...
        # Get the sorted set size to know how many elements to expect
        card_response = self._send_command(f"ZCARD {key}")
        card = int(card_response)
        ...
        if start < 0:
            start = max(0, card + start)
        if stop < 0:
            stop = card + stop
        else:
            stop = min(stop, card - 1)
        
        expected_elements = max(0, stop - start + 1)
        if expected_elements == 0:
            return []
        
        # If withscores, we get pairs
        expected_lines = expected_elements * 2 if withscores else expected_elements
        
        lines = self._parse_array_response(command, expected_lines)
    ...
    def xrange(self, key: str, start: str = "-", end: str = "+", count: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        # For streams, the response format is complex
